...
export function windows(entrypath: string) {
    entrypath = normalize(entrypath).replace(regexs.UNIX_SEP, strings.WINDOWS_SEP);
...
export function unix(entrypath: string) {
